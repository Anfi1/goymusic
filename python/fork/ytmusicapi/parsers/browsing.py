...
def parse_menu_tokens(result: JsonDict) -> tuple:
    tokens = {'pin': None, 'unpin': None, 'notInterested': None}
    is_pinned = False
    like_status = None
    album_id = None
    playlist_id = None
    ...
    if 'menu' in result:
        # Check musicMenuItemRenderer or navigationItemRenderer
        items = nav(result, MENU_ITEMS, True) or []
        for item in items:
            # 1. Check for Feedback/Pin tokens
            if 'menuServiceItemRenderer' in item:
                renderer = item['menuServiceItemRenderer']
                icon = nav(renderer, ['icon', 'iconType'], True)
            ...
            # 2. Check for Toggles (Pin/Unpin and Like)
                ...
                if default_icon == 'UNFAVORITE' or (default_icon == 'FAVORITE' and is_toggled):
                    like_status = 'LIKE'
                elif toggled_icon == 'UNFAVORITE' and is_toggled:
                    like_status = 'LIKE'
                # B. Albums/Playlists (Secondary/Fallback)
                elif like_status is None:
                    if default_icon == 'BOOKMARK' or (default_icon == 'BOOKMARK_BORDER' and is_toggled):
                        like_status = 'LIKE'
                    elif toggled_icon == 'BOOKMARK' and is_toggled:
                        like_status = 'LIKE'

                # --- PINNED STATUS ---
                def_token = nav(renderer, ['defaultServiceEndpoint', 'feedbackEndpoint', 'feedbackToken'], True)
                tgl_token = nav(renderer, ['toggledServiceEndpoint', 'feedbackEndpoint', 'feedbackToken'], True)
                if default_icon == 'KEEP':
                    is_pinned = False
                    tokens['pin'] = def_token
                    tokens['unpin'] = tgl_token
                elif default_icon == 'KEEP_OFF' or toggled_icon == 'KEEP':
                    is_pinned = True
                    tokens['unpin'] = def_token
                    tokens['pin'] = tgl_token

            # 3. Check for Navigation (Album/Playlist links)
            elif 'menuNavigationItemRenderer' in item:
                renderer = item['menuNavigationItemRenderer']
                n_id = nav(renderer, ['navigationEndpoint', 'browseEndpoint', 'browseId'], True)
                if n_id:
                    if n_id.startswith('MPREb'):
                        album_id = n_id
                    elif n_id.startswith('PL') or n_id.startswith('VL'):
                        playlist_id = n_id

    return tokens, is_pinned, like_status, album_id, playlist_id
                    ...
                    if content:
                        ...
                        if not content.get('artists'):
                            for r in subtitle_runs:
                                t = r['text'].strip()
                                if t and t not in ['•', '/', '&', ','] and not t.isdigit() and t.lower() not in ['album', 'single', 'ep', 'альбом', 'сингл', 'песня', 'video', 'playlist']:
                                    if 'artists' not in content: content['artists'] = []
                                    content['artists'].append({'name': t, 'id': r.get('navigationEndpoint', {}).get('browseEndpoint', {}).get('browseId')})
