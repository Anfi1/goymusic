...
def parse_header_menu_data(header: JsonDict) -> JsonDict:
    res = {"likeStatus": "INDIFFERENT", "menu_tokens": None, "isPinned": False}
    buttons = header.get("buttons", [])

    # 1. Like Status (Toggle Button)
    ...
    if toggle_button:
        ...
        if default_icon == "BOOKMARK" or (default_icon == "BOOKMARK_BORDER" and is_toggled):
            ...
            if service:
                res["likeStatus"] = parse_like_status(service)
    ...
    if menu:
        ...
        res["menu_tokens"] = tokens
        res["isPinned"] = pinned
        if home_like and res["likeStatus"] == "INDIFFERENT":
            res["likeStatus"] = home_like
...
def parse_playlist_items(
    ...
    # Pass 1: Pre-populate artist cache with explicit artist IDs in this list
    for result in results:
        if MRLIR not in result:
            continue
        data = result[MRLIR]
        for flex in data.get("flexColumns", []):
            flex_item = flex.get("musicResponsiveListItemFlexColumnRenderer")
            runs = flex_item.get("text", {}).get("runs", []) if flex_item else []
            for run in runs:
                if isinstance(run, dict) and "navigationEndpoint" in run:
                    bid = nav(run, NAVIGATION_BROWSE_ID, True)
                    if bid and not (bid.startswith("MPRE") or "release_detail" in bid):
                        name = run.get("text")
                        if name:
                            store(name, bid)
