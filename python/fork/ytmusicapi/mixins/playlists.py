...
class PlaylistsMixin(MixinProtocol):
    def get_playlist(
        ...
        # Extract likeStatus and pinning tokens from the header
        header_menu = parse_header_menu_data(header)
        playlist.update(header_menu)
        ...
        if "contents" in content_data:
            ...
            if limit is not None and limit > len(playlist["tracks"]):
                parse_func = lambda contents: parse_playlist_items(contents, is_collaborative=is_collaborative)
                playlist["tracks"].extend(
                    get_continuations_2025(content_data, limit - len(playlist["tracks"]), request_func_continuations, parse_func)
                )
                # Update token after fetching continuations
                playlist["continuation"] = get_continuation_token(content_data["contents"]) if "contents" in content_data else None
