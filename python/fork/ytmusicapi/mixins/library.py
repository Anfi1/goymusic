...
class LibraryMixin(MixinProtocol):
    ...
    def get_account_info(self) -> JsonDict:
        ...
        # Extract channelId from the first menu item (usually "Your channel")
        channel_id = None
        try:
            sections = nav(response, ["actions", 0, "openPopupAction", "popup", "multiPageMenuRenderer", "sections"])
            for section in sections:
                items = nav(section, ["multiPageMenuSectionRenderer", "items"])
                for item in items:
                    link = item.get("compactLinkRenderer")
                    if link and nav(link, ["icon", "iconType"], True) == "ACCOUNT_BOX":
                        channel_id = nav(link, ["navigationEndpoint", "browseEndpoint", "browseId"])
                        break
                if channel_id:
                    break
        except:
