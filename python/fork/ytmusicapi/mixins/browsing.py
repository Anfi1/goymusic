...
class BrowsingMixin(MixinProtocol):
    ...
    def get_artist(self, channelId: str) -> JsonDict:
        ...
        if descriptionShelf:
            # DESCRIPTION_RUN_LIST isn't defined in this fork's navigation.py yet;
            # inline the equivalent path (mirrors upstream's DESCRIPTION_RUN_LIST = ["description", "runs"]).
            description, description_runs = parse_description_runs(nav(descriptionShelf, ["description", "runs"], True))
    ...
    def get_album(self, browseId: str) -> JsonDict:
        ...
        # Extract continuation token for more tracks
        from ytmusicapi.continuations import get_continuation_token
        album["continuation"] = get_continuation_token(results["contents"])
