...
class SearchMixin(MixinProtocol):
    def search(
        ...
        # Extract spelling correction if any
        correction = None
        try:
            # Path for corrections is usually in the first section of the results
            tabs = nav(response, ['contents', 'tabbedSearchResultsRenderer', 'tabs'], True)
            if tabs:
                sections = nav(tabs[0], ['tabRenderer', 'content', 'sectionListRenderer', 'contents'], True)
                if sections:
                        ...
                        if 'itemSectionRenderer' in section:
                            ...
                            for item in contents:
                                if 'didYouMeanRenderer' in item:
                                    runs = nav(item['didYouMeanRenderer'], ['correctedQuery', 'runs'], True)
                                    if runs: correction = "".join([r['text'] for r in runs])
                                    break
                                if 'showingResultsForRenderer' in item:
                                    runs = nav(item['showingResultsForRenderer'], ['correctedQuery', 'runs'], True)
                                    if runs: correction = "".join([r['text'] for r in runs])
                                    break
                        if correction: break
        except: pass

        def get_search_results(results, correction):
            return {"results": results, "correction": correction} if correction else results

        # no results
        if "contents" not in response:
            return get_search_results(search_results, correction)
