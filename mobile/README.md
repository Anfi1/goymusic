...
Десктоп сверстан гридом с жёстким минимумом 1024x700 (`--app-min-width` в
`src/styles/theme.css`, `minWidth` окна в `electron/main.ts`). На телефоне этот минимум
растягивает страницу шире экрана — отсюда уехавший плеер. `mobile.css` снимает минимум
и перестраивает грид в одну колонку (контент + плеер), сайдбар уезжает в выдвижную
панель, кнопка вызова которой создаётся в `main.tsx`.

Тонкости, которые стоили времени:

- **`:root:root` не опечатка.** `mobile.css` импортируется раньше `theme.css`, и при
  равной специфичности выигрывала тема — высота плеера оставалась десктопной.
- **Классы CSS-модулей неуникальны** (`.container` есть у половины компонентов), по ним
  не прицелиться. `main.tsx` помечает плеер и сайдбар своими классами.
- **Обложка в плеере**: `<img>` внутри задан не в процентах и вылезал из уменьшенной
  обёртки на строку прогресса — ужимается отдельным правилом.
- **Мост обязан повторять preload целиком.** Любой недостающий метод роняет экран в
  ErrorBoundary: `window.bridge.getSongsPath is not a function`. Покрыты все 37.
