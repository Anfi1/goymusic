...
const HistorySearch = memo(
  ({ onSearch }: { onSearch: (q: string) => void }) => {
...
const HistoryTable = React.forwardRef(
  ({ context, ...props }: any, ref: any) => (
...
const HistoryTableRow = React.forwardRef((props: any, ref: any) => {
  ...
  const [isActive, setIsActive] = useState(
    player.currentTrack?.id === item?.entry?.track?.id
  );

  useEffect(() => {
    if (!item?.entry?.track?.id) return;
    // Фибра переиспользуется react-virtuoso при скролле — initializer useState
    // не перезапускается, поэтому пересчитываем сразу перед подпиской.
    setIsActive(player.currentTrack?.id === item?.entry?.track?.id);
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent) => {
      if (item?.type === 'track') {
        e.preventDefault();
        context.onContextMenu(e, item.entry.track, {
...
const HistoryRowContent = memo(
  ({
    ...
    const entry = item.type === 'track' ? item.entry : null;
    const filteredIdx = item.type === 'track' ? item.index : -1;
    ...
    const extraCells = useMemo(
      () => [
        <td key="played-at" className={styles.timeCell}>
          {timeStr}
        </td>,
      ],
      [timeStr]
    ...
    if (item.type === 'header')
      return (
        <td colSpan={5} className={styles.dateCell}>
          {item.date}
        </td>
      );
      ...
      try {
        ...
        const hydratedData = data.map((entry) => {
          ...
          if (isSCTrack && entry.track.scId && scLikedIds.has(entry.track.scId)) {
            ...
            return {
              ...entry,
              track: { ...entry.track, likeStatus: 'INDIFFERENT' as const },
            };
          }
          return entry;
        });
        ...
        if (e.detail.status === 'success') {
          setHistory((prev) => {
            ...
            const next = prev.map((item) => {
              if (
                item.track.id === e.detail.id &&
                item.track.likeStatus !== e.detail.likeStatus
              ) {
                changed = true;
                return {
                  ...item,
                  track: { ...item.track, likeStatus: e.detail.likeStatus },
                };
              }
              return item;
            });
    ...
    const filteredHistory = useMemo(() => {
      ...
      return rawHistory.filter((item) => {
        if (q) {
          ...
          const artistMatch = item.track.artists?.some((a) =>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.controls}>
          ...
          <div className={styles.filterGroup}>
            <button
              className={`${styles.filterBtn} ${showOnlyLiked ? styles.filterBtnActive : ''}`}
              onClick={() => setShowOnlyLiked(!showOnlyLiked)}
            >
              <Heart size={14} fill={showOnlyLiked ? 'currentColor' : 'none'} />
              <span>Liked</span>
            </button>
            ...
            <div className={styles.dateFilterWrapper}>
              <button
                className={`${styles.filterBtn} ${dateRange.start ? styles.filterBtnActive : ''}`}
        ...
        <div className={styles.listContainer}>
          {isLoading ? (
            ...
            <TableVirtuoso
              ...
              itemContent={(idx, item, ctx) => (
                <HistoryRowContent
                  item={item}
                  onSelectArtist={ctx?.onSelectArtist}
                  onSelectAlbum={ctx?.onSelectAlbum}
                />
              )}
        ...
        {showCharts && (
          <HistoryCharts
            ...
            onSelectArtist={onSelectArtist}
            onSelectAlbum={onSelectAlbum}
            onPlayTrack={(track) => player.playSingle(track, 'history')}
          />
        )}
      </div>
    );
