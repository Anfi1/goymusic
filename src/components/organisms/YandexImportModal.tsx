...
import { yandexImportStreaming } from '../../api/yt';
import { YandexTrackRow, YandexTrackItem } from './YandexTrackRow';
import styles from './YandexImportModal.module.css';

type Stage = 'idle' | 'running' | 'done' | 'failed';

interface YandexImportModalProps {
    isOpen: boolean;
    onClose: () => void;
}

export const YandexImportModal: React.FC<YandexImportModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleStart = async (startIndex: number = 0) => {
        ...
        const unlisten = (window as any).bridge.onPyEvent((msg: any) => {
            if (msg.event === 'yandex_import_total') {
                setTotal(msg.total);
            } else if (msg.event === 'yandex_track_done') {
        ...
        try {
            await yandexImportStreaming(token.trim(), startIndex, controller.signal);
    ...
    const handlePause = () => {
        ...
        // Snapshot resume index before unlisten (no more events after this)
    ...
    return createPortal(
        <div className={styles.overlay} onClick={handleClose}>
            <div className={styles.modal} onClick={e => e.stopPropagation()}>
                <h3 className={styles.title}>Import from Yandex Music</h3>

                {stage === 'idle' && (
                    <p className={styles.subtitle}>
                        Paste your Yandex Music token. Liked tracks will be searched on YouTube Music and liked if artist + title match at 90%.
                    </p>
                )}
                ...
                {(stage === 'idle' || stage === 'failed') && (
                    <div className={styles.inputRow}>
                        <input
                            ...
                            type="password"
                            placeholder="Yandex Music token"
                            value={token}
